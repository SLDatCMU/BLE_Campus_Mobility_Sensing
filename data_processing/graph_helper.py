...
from analysis import edge_daliy_analysis

#node = [2,3,4,5,6,7,8,9,10,12,13,14,16,18,19,20,21,22,23,24,25,26]
node = [2,3,4,5,6,7,8,9,12,13,14,16,18,20,21,23,24,25,26,28]
...
def get_min_length(device_list):
    ...
    for list in device_list:
        print len(list)
        if (min_len > len(list)) & (len(list) > 1000):
            min_len = len(list)
    return min_len
...
def edge_list_to_csv(edge_list, input_date, path, label):
    """
        Write the node list to Gephi csv format dynamic graph
        Args:
    ...
    loc = path + '/graph/'
    f = open(loc + input_date + '_edge_' + label + '.csv', 'w')
...
def pure_population(desired_time, device_list, node, connect_time):
    result = {}
    length = len(device_list)
    desired_time = int(desired_time)
    for i in range(length):
        tmp = 0
        for t in range(desired_time, desired_time + connect_time):
            tmp += len(device_list[i][t])
        result[str(node[i])] = int(tmp / 15)
    return result
...
def main():
    ...
    for i in range(6, 19):
        ...
        for j in range (0, 4):
            ...
            node_list.append(pure_population(time_switch(desired_time, time_slot), device_list, node, connect_time * 6 / time_slot))
            edge_list.append(dict)

    connectivity_analysis(edge_list, input_date)
    node_list, node_layer_wise = collection_analysis_node(node_list)
    node_list_to_csv(node_list, input_date, path, "collective")
    #    for i in range(len(node_layer_wise)):
    #        node_list_to_csv(node_layer_wise[i], input_date, path, "inside" + str(i))
    edge_list, edge_inside, edge_inter = collection_analysis_edge(edge_list)
#    _, max, medium = edge_daliy_analysis(edge_list, path, input_date)
    edge_list_to_csv(edge_list, input_date, path, "collective")
    #    for i in range(len(edge_inside)):
    #        edge_list_to_csv(edge_inside[i], input_date, path, "inside" + str(i))
    #    edge_list_to_csv(edge_inter, input_date, path, "inter")

def edge_analysis(input_date, time_slot, path, connect_time):
    device_list = get_device_list(node, input_date, time_slot, path)
    remove_duplicate_device(device_list)
    edge_list = []
    node_list = []
